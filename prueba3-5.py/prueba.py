...
def nuevaVentana():							#Funcion nuevaVentana ... Nos permitira el registro de nuevos usuarios
	...
	newVentana.geometry("280x450+300+250")	#Tamaño de la ventana
...
def mensaje():
    ...
    messagebox.showwarning("Castro Camilo", "Sistema de ventas desarrollado en Python 3.7")
...
def conexionBBDD():
    ...
    miConexion=sqlite3.connect("DatosDB")
    miCursor=miConexion.cursor()
    
    try:
        
        miCursor.execute('''
            CREATE TABLE DATOSECUNDARIOS (
            DOCUMENTO INTEGER PRIMARY KEY AUTOINCREMENT,
...
def crear():
    ...
    datos=miProvincia.get(),miMetodo.get(),miTotal.get()
    miCursor.execute("INSERT INTO DATOSECUNDARIOS VALUES(NULL,?,?,?)", (datos))
...
def leer():
    ...
    for usuario in elUsuario:
        ...
        miProvincia.set(usuario[0])
        miMetodo.set(usuario[1])
        miTotal.set(usuario[3])
...
def actualizar():
    ...
    miCursor.execute("UPDATE DATOSECUNDARIOS SET PROVINCIA=?, METODO=?, TOTAL=?"+
...
def borrar():
    ...
    miConexion=sqlite3.connect("DatosDB")
    miCursor=miConexion.cursor()
    miCursor.execute("DELETE FROM DATOSECUNDARIOS WHERE ID=" +miDocumento.get())
...
l1=Label(root, text="Provincia: ")
l1.grid(column=0, row=1)


e1=Entry(root, textvariable=miProvincia)
e1.grid(column=1, row=1)

l2=Label(root, text="Metodo de Pago: ")
l2.grid(column=0, row=2)


e2=Entry(root, textvariable=miMetodo)
e2.grid(column=1, row=2)

l3=Label(root, text="Total:   $ ")
l3.grid(column=0, row=3)

e3=Entry(root,textvariable=miTotal)
e3.grid(column=1, row=3)
